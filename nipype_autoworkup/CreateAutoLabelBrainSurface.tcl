    ...
    ##############################################################################
    proc CreateAutoLabelBrainSurface {BrainMaskFile WarpedCerebellumFile HemisphereMaskFile TissueClassFile \
      ...
      if {[Brains::Utils::CheckOutputsNewer \
        ...
        set antiAliasFlag 1
        set measureDepth 0
        set measureCurvature 0
        set measureConvexHullDepth 0
        ...
        Brains::External::runBrainsCortexGenusZero $OutputImageFilename $OutputSurfaceFilename $surfaceValue \
          $depthValue $highValue $contourValue $maxRMSError $targetReduction $numIterations $passBand \
          $maxDepth $antiAliasFlag $measureDepth $measureCurvature $measureConvexHullDepth \
          $preserveTopology $boundarySmoothing $normalizedCoordinates $boundarySmoothing $keepLargestConnected
        }
