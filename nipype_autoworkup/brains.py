...
class BRAINS4CommandLine(CommandLine):
    """Experimental BRAINS wrapper. Work in progress.

    """

    input_spec = BRAINS4CommandLineInputSpec
    ...
    def _outputs(self):
        base = super(BRAINS4CommandLine, self)._outputs()
    ...
    def __init__(self, module, **inputs):
        fullpath=module   ## store the full path for command processing
        module=os.path.basename(module)  ## Overwrite module with the shortname
        warnings.warn('BRAINS is Not fully implemented', RuntimeWarning)
        super(BRAINS4CommandLine, self).__init__(command=fullpath, name= module, **inputs)
        dom = self._grab_xml(fullpath)
    ...
    def _format_arg(self, name, spec, value):
        ...
        return super(BRAINS4CommandLine, self)._format_arg(name, spec, value)
