...
void vtkKWDICOMExporter::CreateWidget()
{
  ...
  this->ButtonEditDICOMInfo->SetImageToPixels(image_cd,
                                              image_cd_width,
                                              image_cd_height,
                                              image_cd_pixel_size,
                                              image_cd_length);
  ...
  //   this->ButtonPropagateDICOMInfo->SetImageToPixels(image_cd,
  // image_cd_width, image_cd_height, image_cd_pixel_size, image_cd_length);
  //   this->ButtonPropagateDICOMInfo->SetCommand(this,
  // "PropagateDICOMInfoCallback");
  //   this->ButtonPropagateDICOMInfo->SetBalloonHelpString("propagate
  // highlighted image DICOM information to all other images of the exam");
  ...
  //   this->ButtonSetOutputDirectory->SetImageToPixels(image_saveas,
  // image_saveas_width, image_saveas_height, image_saveas_pixel_size,
  // image_saveas_length);
  //   this->ButtonSetOutputDirectory->SetCommand(this,
  // "SetOutputDirectoryCallback");
  //   this->ButtonSetOutputDirectory->SetBalloonHelpString("Set the output
  // directory for the DICOM exam");
...
void vtkKWDICOMExporter::EditDICOMInfoCallback()
{
  ...
  if ( sequence )
    {
    vtkKWPopupWarningMessage (
      this,
      "Unfortunatelly there is no possibility \nto set DICOM flags to all frames of a sequence.");

    //     for (unsigned int i=0; i<sequence->GetNumberOfMetaDataSets(); i++)
    //     {
    //       vtkMetaImageData* frame =
    // vtkMetaImageData::SafeDownCast(sequence->GetMetaDataSet (i));
    //       if (!frame)
    //  continue;
