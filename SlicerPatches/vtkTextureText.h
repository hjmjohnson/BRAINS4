...
public:
  ...
  // this method also returns TextureTextPolys, but using more standard VTK
  // naming
  ...
  // The text goes to the next line when the x position of the next word goes
  // outside the
  ...
  // static vtkFloatingPointType CalculateLineLength(char *string, char
  // *fontname);

  // v12 - this doesn't seem to be used internally, but can be a useful
  // utility...
...
protected:
  ...
  //  Right, Centered seem to work when charSpace and indent are off.  Justified
  // is still a bit off.
  // VTK_TEXT_ALIGNMENT_MONOSPACE - seems to correctly monospace, but too spread
  // out, and w/ multiple columns, they're overlapping w/o wrapping soon enough
  //   especially messed up if SetCharacterSpace() if used - also when indent
  // used
